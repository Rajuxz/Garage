...
from .core import predict_requests
import json
...
def admin_customer_view(request):
    predictions = predict_requests()
    predictions_data = [{'date': date, 'predicted_requests': predicted_requests} for date, predicted_requests in predictions]

    labels = [str(date.date()) for date, _ in predictions]  # Extracting and formatting dates
    data = [int(requests) for _, requests in predictions]
    context = {
         'predictions': predictions_data,
          'labels':json.dumps(labels),
          'data':json.dumps(data)
    }
    return render(request,'vehicle/admin_customer.html',context=context)
...
def download_daily_report(request):
    ...
    reports = models.Request.objects.filter(date=today)
    response = HttpResponse(content_type = 'text/csv')
    formatted_today = today.strftime('%Y-%m-%d')
    response['Content-Disposition'] = f'attatchment; filename="daily_report{today}.csv"'
    writer = csv.writer(response)
     # Add a title row (heading) for the report
    writer.writerow(['Daily Report'])  # Title of the report
    writer.writerow(['Date:', formatted_today])  # Date of the report
    writer.writerow([]) 

    writer.writerow(['Vehicle Name', 'Vehicle No', 'Date', 'Cost'])

    total_cost = 0 
    for report in reports:
        writer.writerow([report.vehicle_name, report.vehicle_no, report.date, report.cost])
        total_cost += report.cost  # Accumulate the total cost
...
def download_weekly_report(request):
        ...
        # Calculate the start date for the weekly report (7 days ago)
        start_date = today - timedelta(days=7)

        # Filter reports for the past week
        reports = models.Request.objects.filter(date__range=[start_date, today])
        ...
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="weekly_report_{formatted_today}.csv"'
        # Create a CSV writer
        writer = csv.writer(response)

        # Add a title row (heading) for the report
        writer.writerow(['Weekly Report'])  # Title of the report
        writer.writerow(['Date Range:', f"{start_date} to {today}"])  # Date range of the report
        writer.writerow([])  # Blank row for spacing

        # Write the CSV header
        writer.writerow(['Vehicle Name', 'Vehicle No', 'Date', 'Cost'])  # CSV header

        total_cost = 0  # Initialize total cost

        # Write the data rows and calculate total cost
        for report in reports:
            writer.writerow([report.vehicle_name, report.vehicle_no, report.date, report.cost])
            total_cost += report.cost  # Accumulate the total cost

        # Write the total cost at the end of the report
        writer.writerow([])  # Blank row for spacing
        writer.writerow(['Total Amount', '', '', total_cost])  # Total cost row
...
def admin_report_view(request):
    ...
    total_cost = reports.aggregate(total=Sum('cost'))['total'] or 0 
